...
// The topology simulated in this experiment involves 64 hosts in a fat tree.
// 1 ToR switch is connected to 8 hosts with 100G links. There are 8 ToR
// switches. 1 Leaf switch is connected to 2 ToRs with 400G links. There are 8
// Leaf switches. 1 Spine switch is connected to 4 of the Leaf switches with
// 400G links. There are 4 Spine switches.
...
NS_LOG_COMPONENT_DEFINE("Bolt64HostsFattreeSimulation");
...
int main(int argc, char *argv[]) {
  ...
  cmd.AddValue("note", "Any note to identify the simulation in the future",
               simNote);
  cmd.AddValue("use10GLinks", "A faster but less realistic simulation",
               use100GLinks);
  ...
  LogComponentEnable("Bolt64HostsFattreeSimulation", LOG_LEVEL_WARN);
  ...
  if (debugMode) {
    ...
    LogComponentEnable("Bolt64HostsFattreeSimulation", LOG_LEVEL_DEBUG);
  ...
  std::string tracesFileName("outputs/bolt-64host-fattree/");
  ...
  NetDeviceContainer leafToSpineDevices[nLeafSw * nSpinePerLeaf];
  for (int i = 0; i < nLeafSw; i++) {
    for (int j = 0; j < nSpinePerLeaf; j++) {
      int spineNodeIdx = (i % nSpinePerLeaf) * nSpinePerLeaf + j;
      leafToSpineDevices[i * nSpinePerLeaf + j] =
          coreLinks.Install(leafNodes.Get(i), spineNodes.Get(spineNodeIdx));
      for (uint32_t n = 0; n < leafToSpineDevices[i * nSpinePerLeaf + j].GetN();
           n++)
        leafToSpineDevices[i * nSpinePerLeaf + j].Get(n)->SetMtu(mtu);
    }
  }
  ...
  boltQdisc.SetRootQueueDisc(
      ...
      IntegerValue(mtu * 2), "EnableAbs", BooleanValue(enableAbs));
  ...
  QueueDiscContainer leafToSpineQdisc[nLeafSw * nSpinePerLeaf];
  for (int i = 0; i < nLeafSw * nSpinePerLeaf; i++) {
  ...
  Ipv4InterfaceContainer LeafToSpineIfs[nLeafSw * nSpinePerLeaf];
