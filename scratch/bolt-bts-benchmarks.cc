...
// flow joins one by one. Then statistics about the flows (ie. cwnd, rtt
// etc.) are collected to see how different algorithms react to the change.
//
// Example to run the simulation:
// $./waf --run "scratch/bolt-bts-microbencmarks --ccModr=SWIFT"
...
NS_LOG_COMPONENT_DEFINE("BoltBtsBenchmarks");
...
static void BtsDepartureTrace(Ptr<OutputStreamWrapper> stream,
  ...
  Time now = Simulator::Now();
  NS_LOG_INFO("BTS send " << now.GetNanoSeconds() << " " << nBtsInFlight << " "
                          << curQLen);

  *stream->GetStream() << now.GetNanoSeconds() << " " << nBtsInFlight << " "
                       << curQLen << std::endl;
}
...
int main(int argc, char *argv[]) {
  ...
  cmd.AddValue("stopFlowsManually",
               ...
               stopFlowsManually);
  cmd.AddValue("simBothArrivalAndDeparture",
               "Whether to simulate both arrival and departure of a flow.",
               simBothArrivalAndDeparture);
  ...
  LogComponentEnable("BoltBtsBenchmarks", LOG_LEVEL_WARN);
  ...
  if (debugMode) {
    LogComponentEnable("BoltBtsBenchmarks", LOG_LEVEL_DEBUG);
  ...
  std::string tracesFileName("outputs/bolt-bts-benchmarks/");
  if (debugMode)
  ...
  if (simBothArrivalAndDeparture) tracesFileName += "_arrAndDep";
  if (!simNote.empty()) {
  ...
  std::string btsTracesFileName = tracesFileName + ".bts";
  ...
  if (traceBtsDeparture) {
    Ptr<OutputStreamWrapper> btsTraceStream =
        asciiTraceHelper.CreateFileStream(btsTracesFileName);
    bottleneckQdisc.Get(0)->TraceConnectWithoutContext(
        "BtsDeparture", MakeBoundCallback(&BtsDepartureTrace, btsTraceStream));
  }
  ...
  double dataToPktRatio =
      static_cast<double>(payloadSize) / static_cast<double>(MTU);

  /******** Schedule the long lived flows for microbenchmarks ********/
  std::vector<uint32_t> flowSizesBytes;
  if (simBothArrivalAndDeparture) {
    NS_ASSERT_MSG(
        nFlows == 2,
        "Currently, this mode of simulation can handle only 2 flow cases!");
    flowSizesBytes.push_back(21875000);
    flowSizesBytes.push_back(3318750);
  } else {
    uint32_t baseFlowSize =
        static_cast<uint32_t>(newFlowTime * static_cast<double>(bottleneckBps) *
                              dataToPktRatio / 8.0);
    for (int i = 0; i < nFlows; i++)
      flowSizesBytes.push_back(baseFlowSize * (nFlows - i));
  }
  ...
  for (int i = 0; i < nFlows; i++) {
    receiverAddr = InetSocketAddress(receiverToSwitchIfs[i].GetAddress(0),
                                     receiverPortNoStart + i);

    receiverSocket[i] = Socket::CreateSocket(receiverNodes.Get(i),
    ...
    senderSocket[i]->Bind(InetSocketAddress(senderToSwitchIfs[i].GetAddress(0),
                                            senderPortNoStart + i));
    Simulator::Schedule(Seconds(START_TIME + i * newFlowTime),
                        &SendLongLivedFlow, senderSocket[i], receiverAddr,
                        flowSizesBytes[i]);
  }
  ...
  double duration = 0.0;
  if (stopFlowsManually) duration = (nFlows * 2 - 1) * newFlowTime;
  Config::ConnectWithoutContext(
  ...
  if (stopFlowsManually)
    Simulator::Stop(Seconds(START_TIME + (nFlows * 2 - 1) * newFlowTime));
  Simulator::Run();
  Simulator::Destroy();
