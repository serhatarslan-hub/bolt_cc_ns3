...
public:
  ...
  void Install(Ptr<Node> node, std::vector<InetSocketAddress> remoteClients);

  void SetWorkload(double load, std::map<double, int> msgSizeCDF,
                   double avgMsgSizePkts, uint64_t bottleneckBps = 0);
 ...
 private:
  ...
  std::vector<InetSocketAddress>
      m_remoteClients;  //!< List of clients that this app can send to
  std::map<double, int>
      m_msgSizeCDF;  //!< The CDF of msg sizes {cum. prob. -> msg size in pkts}

  Ptr<ExponentialRandomVariable>
      m_interMsgTime;  //!< rng for rate of message generation in sec/msg
  Ptr<UniformRandomVariable>
      m_msgSize;  //!< rng to choose msg size from the set workload
  Ptr<UniformRandomVariable>
      m_remoteClient;  //!< rng to choose remote client to send msg to
  ...
  uint32_t m_staticMsgSize;   //!< Sets message sizes to a constant value if
                              //!< provided for testing purposes
  bool m_unitsInBytes; //!< Whether units in workload are in bytes vs packets
};
