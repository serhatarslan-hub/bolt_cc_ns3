...
uint16_t Buffer::Iterator::ReadNtohU16(void) {
  uint8_t *buffer;
  if (m_current + 2 <= m_zeroStart) {
    buffer = &m_data[m_current];
...
uint32_t Buffer::Iterator::ReadNtohU32(void) {
  uint8_t *buffer;
  if (m_current + 4 <= m_zeroStart) {
    buffer = &m_data[m_current];
  } else if (m_current >= m_zeroEnd) {
...
uint8_t Buffer::Iterator::PeekU8(void) {
...
uint8_t Buffer::Iterator::ReadU8(void) {
  uint8_t ret = PeekU8();
  m_current++;
  return ret;
}

uint16_t Buffer::Iterator::ReadU16(void) {
...
uint32_t Buffer::GetSize(void) const { return m_end - m_start; }

Buffer::Iterator Buffer::Begin(void) const {
  NS_ASSERT(CheckInternalState());
