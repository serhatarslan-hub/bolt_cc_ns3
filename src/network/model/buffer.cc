...
struct Buffer::Data *Buffer::Allocate(uint32_t reqSize) {
...
uint32_t Buffer::GetInternalSize(void) const {
  NS_LOG_FUNCTION(this);
  return m_zeroAreaStart - m_start + m_end - m_zeroAreaEnd;
}
uint32_t Buffer::GetInternalEnd(void) const {
  NS_LOG_FUNCTION(this);
...
void Buffer::AddAtEnd(const Buffer &o) {
  ...
  if (m_data->m_count == 1 && m_end == m_zeroAreaEnd &&
...
Buffer Buffer::CreateFullCopy(void) const {
  NS_LOG_FUNCTION(this);
  NS_ASSERT(CheckInternalState());
...
uint8_t const *Buffer::PeekData(void) const {
  NS_LOG_FUNCTION(this);
  NS_ASSERT(CheckInternalState());
...
uint32_t Buffer::Iterator::GetDistanceFrom(Iterator const &o) const {
  NS_LOG_FUNCTION(this << &o);
...
uint32_t Buffer::Iterator::ReadU32(void) {
  NS_LOG_FUNCTION(this);
...
uint64_t Buffer::Iterator::ReadU64(void) {
  NS_LOG_FUNCTION(this);
  uint8_t byte0 = ReadU8();
  uint8_t byte1 = ReadU8();
  uint8_t byte2 = ReadU8();
...
uint16_t Buffer::Iterator::SlowReadNtohU16(void) {
  NS_LOG_FUNCTION(this);
  uint16_t retval = 0;
  retval |= ReadU8();
  retval <<= 8;
...
uint32_t Buffer::Iterator::SlowReadNtohU32(void) {
  NS_LOG_FUNCTION(this);
  uint32_t retval = 0;
  retval |= ReadU8();
  retval <<= 8;
...
uint64_t Buffer::Iterator::ReadNtohU64(void) {
  NS_LOG_FUNCTION(this);
  uint64_t retval = 0;
  retval |= ReadU8();
  retval <<= 8;
...
uint16_t Buffer::Iterator::ReadLsbtohU16(void) {
  NS_LOG_FUNCTION(this);
  uint8_t byte0 = ReadU8();
  uint8_t byte1 = ReadU8();
  uint16_t data = byte1;
...
uint32_t Buffer::Iterator::ReadLsbtohU32(void) {
  NS_LOG_FUNCTION(this);
  uint8_t byte0 = ReadU8();
  uint8_t byte1 = ReadU8();
  uint8_t byte2 = ReadU8();
...
uint64_t Buffer::Iterator::ReadLsbtohU64(void) {
  NS_LOG_FUNCTION(this);
  uint8_t byte0 = ReadU8();
  uint8_t byte1 = ReadU8();
  uint8_t byte2 = ReadU8();
...
uint16_t Buffer::Iterator::CalculateIpChecksum(uint16_t size,
  ...
  for (int j = 0; j < size / 2; j++) sum += ReadU16();

  if (size & 1) sum += ReadU8();

  while (sum >> 16) sum = (sum & 0xffff) + (sum >> 16);
  return ~sum;
}

uint32_t Buffer::Iterator::GetSize(void) const {
  NS_LOG_FUNCTION(this);
  return m_dataEnd - m_dataStart;
}

uint32_t Buffer::Iterator::GetRemainingSize(void) const {
  NS_LOG_FUNCTION(this);
  return m_dataEnd - m_current;
}

std::string Buffer::Iterator::GetReadErrorMessage(void) const {
  NS_LOG_FUNCTION(this);
  std::string str =
...
std::string Buffer::Iterator::GetWriteErrorMessage(void) const {
  NS_LOG_FUNCTION(this);
  std::string str;
...
uint8_t *Buffer::GetBuffer() const { return m_data->m_data + m_start; }
