...
TypeId ByteTagIterator::Item::GetTypeId(void) const { return m_tid; }
uint32_t ByteTagIterator::Item::GetStart(void) const { return m_start; }
uint32_t ByteTagIterator::Item::GetEnd(void) const { return m_end; }
...
bool ByteTagIterator::HasNext(void) const { return m_current.HasNext(); }
ByteTagIterator::Item ByteTagIterator::Next(void) {
...
PacketTagIterator::PacketTagIterator(const struct PacketTagList::TagData *head)
    : m_current(head) {}
bool PacketTagIterator::HasNext(void) const { return m_current != 0; }
PacketTagIterator::Item PacketTagIterator::Next(void) {
  NS_ASSERT(HasNext());
...
PacketTagIterator::Item::Item(const struct PacketTagList::TagData *data)
    : m_data(data) {}
TypeId PacketTagIterator::Item::GetTypeId(void) const { return m_data->tid; }
...
Ptr<Packet> Packet::Copy(void) const {
...
Packet::Packet()
    : m_buffer(),
       ...
       */
      m_metadata(
          static_cast<uint64_t>(Simulator::GetSystemId()) << 32 | m_globalUid,
          0),
      m_nixVector(0) {
  m_globalUid++;
}
...
Packet::Packet(const Packet &o)
  ...
  o.m_nixVector ? m_nixVector = o.m_nixVector->Copy() : m_nixVector = 0;
}
...
void Packet::SetNixVector(Ptr<NixVector> nixVector) { m_nixVector = nixVector; }

Ptr<NixVector> Packet::GetNixVector(void) const { return m_nixVector; }
...
uint64_t Packet::GetUid(void) const { return m_metadata.GetUid(); }
...
PacketMetadata::ItemIterator Packet::BeginItem(void) const {
...
ByteTagIterator Packet::GetByteTagIterator(void) const {
...
PacketTagIterator Packet::GetPacketTagIterator(void) const {
...
uint8_t *Packet::GetBuffer() const { return m_buffer.GetBuffer(); }
